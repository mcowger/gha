...
- `opencode-assistant.yml` handles trusted collaborator requests from issue and PR comments. It
  needs `contents: write` because explicit requests may change code and push it to a PR branch.
- The workflows use the repository's existing `LLM_API_KEY`, `LLM_BASE_URL`, and `LLM_MODEL`
  secrets through an OpenAI-compatible OpenCode provider.
