...
export function generateIndexHtml(reports: Array<{ filename: string; title: string; date: string; projectCount: number; videoUrl: string; sourceLabel?: string }>, lastCheckedAt?: string | null): string {
  const lastCheckedText = lastCheckedAt
    ? new Date(lastCheckedAt).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })
    : null;
...
<div class="header">
  ...
  ${lastCheckedText ? `<p class="last-checked">Last checked: ${escapeHtml(lastCheckedText)}</p>` : ''}
</div>
<div class="list">
${items}
</div>
